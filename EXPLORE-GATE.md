...
### Conditions — both resolved:
1. **HC-7 / DEV_BYPASS_AUTH** — ✅ RESOLVED: `onboarding-agent/src/security/keycloak-auth.ts`
   contains no bypass guard. Comment on line 9 explicitly states HC-7 permanently removed.
   JWT validation is mandatory in all environments.

2. **Brevo API key rotation** — ✅ RESOLVED: Old key (`...YdPffgZAeeYE0cin`) replaced with
   new key (`...DjjThp2fuqnRPGTV`) in both `engage-secrets` (i3-engage) and `pmaas-secrets`
   (i3-pmaas). Both cluster secrets verified len=89 with new suffix. Deployments restarted.
   Old key must be deleted from Brevo dashboard to fully revoke it.

All findings are tracked in the risk register (§12) and implementation sequence (§13).
**Platform is APPROVED to enter Plan phase.**
