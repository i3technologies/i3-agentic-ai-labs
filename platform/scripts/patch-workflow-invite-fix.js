#!/usr/bin/env node
// Patch Send Invite node to handle both snake_case and camelCase field names
// Fix inviteStatus 400: item.body.studentEmail → item.body.studentEmail || item.body.student_email
...
// Flatten body fields for downstream nodes
return [{ json: {
...
// Fixed Admin Alert jsCode — reads flat item.* (already set by upstream nodes)
...
const examCode     = item.examCode     || item.exam_title    || item.examTitle || '';
...
async function main() {
  ...
  if (wfResp.status !== 200) { console.error('Fetch failed:', wfResp.status, wfResp.body); process.exit(1); }
  ...
  console.log('\nDone. Send Invite + Admin Alert patched with snake_case fix.');
}
